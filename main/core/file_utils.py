...
def ensure_hidden_read_first_md(sketch_dir) -> None:
    ...
    try:
        ...
        # 1. Clean up old obsolete duplicate instruction files
        redundant_files = (
            "READ-FIRST.md", ".READ-FIRST.md", "SKILL.md", ".SKILL.md",
            "OPENCODE.md", ".ignore"
        )
        for r_name in redundant_files:
            for parent_d in (cache_dir, s_dir):
                rf = parent_d / r_name
                if rf.is_file() and _is_mcu_generated_instruction_file(rf):
                    try:
                        ensure_file_writable(rf)
                        rf.unlink(missing_ok=True)
                    except Exception:
                        pass

        # 2. Native OpenCode ignore configuration
        # Written at s_dir root so OpenCode CLI immediately respects it and never scans build caches
        ignore_content = (
            "# Auto-generated by MCU Flash GUI for OpenCode AI\n"
            f"{PROJECT_BUILD_CACHE_DIR}/\n"
            f"{PROJECT_BUILD_CACHE_DIR}/**\n"
            f"{AI_PROJECT_STORAGE_DIR}/\n"
            f"{AI_PROJECT_STORAGE_DIR}/**\n"
            "build_artifacts/\n"
            ".build_artifacts/\n"
            ".pio/\n"
        ...
        # 3. Master workspace instructions (AGENTS.md)
        # Clean markdown without frontmatter so OpenCode treats it as global instructions
        ...
        workflow_skill_content = (
            ...
            f"Never search parent directories or edit files in `{PROJECT_BUILD_CACHE_DIR}/`, `.pio/`, or `.opencode/`.\n\n"
        ...
        target_skill_content = (
            ...
            f"- **Selected Board**: `{live_board}`\n"
            f"- **Platform / Architecture**: `{live_platform}`\n"
            f"- **FQBN / Board ID**: `{live_fqbn}`\n"
            ...
            f"- **File Boundary**: Only modify files in `{s_dir.resolve()}` (`*.ino`, `*.cpp`, `*.h`). Never touch cache files in `{PROJECT_BUILD_CACHE_DIR}/`.\n"
        )

        try:
            # Install in .opencode/skills/
            ...
            # Also install in .agents/skills/ for agent tools that discover .agents/
            agents_skills_base = s_dir / ".agents" / "skills"
...
def hide_internal_project_metadata(sketch_dir) -> None:
    ...
    try:
        ...
        # Purge any misplaced hardware state files or stale ready signals from root
        try:
            (s_dir / ".mcu_flasher_project_hardware.json").unlink(missing_ok=True)
            (s_dir / "index_json" / ".mcu_flasher_project_hardware.json").unlink(missing_ok=True)
            (s_dir / ".ai_ready_signal").unlink(missing_ok=True)
        except Exception:
            pass
        ...
        if is_codebase_root:
            ...
            internal_names = [
                ...
                ".mcu_ai_edits", PROJECT_BUILD_CACHE_DIR, "env", "logs", ".opencode",
                ".agents", "compile_commands.json",
            ]
        ...
        if is_codebase_root:
            ...
            compile_commands = s_dir / "compile_commands.json"
            if compile_commands.is_file():
                hide_hidden_attribute(compile_commands)
    except Exception:
        pass
