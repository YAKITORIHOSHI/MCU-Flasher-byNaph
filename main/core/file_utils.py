...
def ensure_hidden_read_first_md(sketch_dir) -> None:
    ...
    try:
        ...
        root_opencode_ign = s_dir / ".opencodeignore"
        try:
            ...
            if not root_opencode_ign.exists() or _is_mcu_generated_instruction_file(root_opencode_ign):
                ensure_file_writable(root_opencode_ign)
                root_opencode_ign.write_text(ignore_content, encoding="utf-8")
                hide_hidden_attribute(root_opencode_ign)
        except Exception:
            pass
        ...
        root_agents_md = s_dir / "AGENTS.md"
        try:
            ...
            if not root_agents_md.exists() or _is_mcu_generated_instruction_file(root_agents_md):
                ensure_file_writable(root_agents_md)
                root_agents_md.write_text(content, encoding="utf-8")
                hide_hidden_attribute(root_agents_md)
        except Exception:
            pass
        ...
        workflow_skill_content = (
            ...
            "description: \"Step-by-step priority workflow for this sketch: 1) Read project_state.json for active board/port/baud, 2) read/edit only root sketch files (*.ino, *.h, *.cpp), 3) read dbs_notif.json on error.\"\n"
            "---\n\n"
            ...
            "Never edit or create files in `.mcu_flasher_build_cache/`, `.pio/`, or `.opencode/`.\n\n"
